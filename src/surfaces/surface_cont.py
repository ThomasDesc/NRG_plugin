...
def vcon(pdb_name,vcon_binary_path):
    string = f'{vcon_binary_path} {pdb_name} > vcon_file.txt'
    os.system(string)
...
def list_file(matrix,output_name):
    ...
    f = open(os.path.join(os.path.dirname(output_name),"List_" + os.path.basename(output_name[:-4]) + ".txt"), "w")
...
def get_args():
    ...
    main(pdb_file, chain1, chain2, output_name, atomtypes_definition, atomtypes_interactions,'.vcon')

def main(pdb_file,chain1,chain2,output_name,atomtypes_definition,atomtypes_interactions,vcon_binary_path):

    res1, res2, chains, atom_numbers = read_residues(pdb_file, chain1, chain2)
    #print (res1, res2)
    ...
    matrix = read_interactions('vcon_file.txt', matrix, chain1, chain2, atomtypes_definition, atomtypes_interactions, atom_numbers, scale_factor)
